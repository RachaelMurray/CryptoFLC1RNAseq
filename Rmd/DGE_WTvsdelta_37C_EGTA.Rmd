---
title: "Differential Gene Expression of WT vs delta at 37°C in YPD + EGTA samples"
...
- `deglist_WTvsdelta_37C_EGTA_up2x_FDR5.txt`
- `deglist_WTvsdelta_37C_EGTA_down2x_FDR5.txt`
...
```{r subset_WTvsdelta_37C_EGTA, fig.height=8, fig.width=8}
sample_sheet_WTvsdelta_37C_EGTA <-
  dplyr::filter(sample_sheet,
                Temperature == "37", Media == "YPD_EGTA")

dds_WTvsdelta_37C_EGTA <- 
  DESeqDataSetFromMatrix(
    countData = counts_all %>%
      dplyr::select(sample_sheet_WTvsdelta_37C_EGTA$SampleName) %>%
      magrittr::set_rownames(counts_all$Gene),
    colData = sample_sheet_WTvsdelta_37C_EGTA,
    design = ~ FLC1) %>%
  DESeq()
...
```{r results_WTvsdelta_37C_EGTA}
results(dds_WTvsdelta_37C_EGTA)

resultsNames(dds_WTvsdelta_37C_EGTA)
```

```{r deseq_df_WTvsdelta_37C_EGTA}
deseq_df_WTvsdelta_37C_EGTA <- 
  biobroom::tidy.DESeqDataSet(dds_WTvsdelta_37C_EGTA) %>%
...
DEGdf_WT_37C_EGTA_up2x_FDR5 <- 
  dplyr::filter(deseq_df_WTvsdelta_37C_EGTA,
                log2FC > 1, padj < 0.05) %>%
  dplyr::arrange(desc(log2FC))
print(DEGdf_WT_37C_EGTA_up2x_FDR5, n = 20)
```
...
# Make data frames of DEGs up and down, arranged by log2FC
DEGdf_WT_37C_EGTA_down2x_FDR5 <- 
  dplyr::filter(deseq_df_WTvsdelta_37C_EGTA,
                log2FC < -1, padj < 0.05) %>%
  dplyr::arrange(log2FC)
print(DEGdf_WT_37C_EGTA_down2x_FDR5, n = 20)
```
...
plot_labels_down <- DEGdf_WT_37C_EGTA_down2x_FDR5 %>%
  dplyr::filter(!is.na(geneName))
```

## Volcano plot of DEGs

```{r volcano_WTvsdelta_37C_EGTA, fig.width = 6, fig.height = 3}
volcano_WTvsdelta_37C_EGTA <- 
 ggplot(data = deseq_df_WTvsdelta_37C_EGTA,
  ...
  geom_point(data = DEGdf_WT_37C_EGTA_up2x_FDR5,
             size = 0.3, colour = "darkblue") +
  geom_point(data = DEGdf_WT_37C_EGTA_down2x_FDR5,
